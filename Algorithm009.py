...
둘째 줄에는 P, M, N가 주어진다. P는 우체부의 도착 시간, M은 우유배달원의 도착 시간, N은 신문배달원의 도착 시간이다. 
...
def count(arrive):
    count = 0
    if (arrive%sum_1) <= A and (arrive%sum_1) != 0:
        count += 1
    if (arrive%sum_2) <= C and (arrive%sum_1) != 0:
        count += 1
    return count
